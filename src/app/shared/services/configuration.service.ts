...
import { RolAdmin, RolClient, RolConsultant } from '../models/role';
...
export class ConfigurationService {
  ...
  public clientUser = new RolClient();
  public consultantUser = new RolConsultant();
  public profiles = [this.adminUser, this.clientUser, this.consultantUser];
  ...
  get role() {
    return this.storageService.getValue('user')?.idRol;
  }
  ...
  public menuInitial = [
    ...
    },
    {
      ...
      children: [
        ...
        {
          text: 'Transferencia entre usuarios',
          icon: '',
          visible: true,
          url: 'transfer',
          select: false,
  ...
  public showBackError(msg: string) {
  ...
  public changeMenu(item: any) {
    ...
    this.router.navigate([
      'home/' +
        item.url +
        (item.url == 'account'
          ? '/' + this.storageService.getValue('user').id
          : ''),
    ]);
  }
  ...
  filterMenu() {
    this.menu = [];
    const menuX = this.menuInitial.filter((item) =>
      item.role?.find((x) => x === this.role)
    );
